...
RSpec.describe Recurrence::BpjsKesehatan::Notifier, type: :model do
  let(:template_detail) { create(:bpjs_kesehatan_recurrence_template_detail) }
  let(:template_detail_id) { template_detail.id }

  let(:template_map) do
    {
      topup_deposit: "olympus_recurrence_topup_deposit_bpjs_kesehatan_payload",
      unsubscribe_success: "olympus_recurrence_unsubscribe_success_bpjs_kesehatan_payload",
      subscribe_success: "olympus_recurrence_subscribe_success_bpjs_kesehatan_payload",
      transaction_success: "olympus_recurrence_transaction_success_bpjs_kesehatan_payload",
      transaction_error: "olympus_recurrence_transaction_error_bpjs_kesehatan_payload"
    }
  end
  ...
  let(:postpaid_product) { 'bpjs_kesehatan' }
  let(:options) { {} }
  ...
  describe 'run!' do
    ...
    context 'when failed' do
      context 'no record' do
        let(:template) { 'olympus_recurrence_topup_deposit_bpjs_kesehatan_payload' }
