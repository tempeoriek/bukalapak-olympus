...
describe Subscribers::SievexPredict do
  include_context 'pubsub_mocks'

  let(:transaction) { build_stubbed(:postpaid_transaction_with_bill, :paid) }
  let(:message) {
    {
  ...
  it 'not raise_error' do
    ...
    expect_any_instance_of(Action::PostpaidTransaction::SievexPredict).to receive(:run!)
