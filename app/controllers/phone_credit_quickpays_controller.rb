class PhoneCreditQuickpaysController < PostpaidsController
  ...
  def index
    ...
    transactions = Quickpays::Get.new('phone-credit-postpaid', today, user_id).run!

    skip_cache = params[:skip_cache] == "true" ? true : false
    results = Quickpays::Inquiry.new('phone-credit-postpaid', today, transactions, skip_cache).run!
  ...
  def delete
    ...
    user_id = decoded_token[:resource_owner_id]
    Quickpays::Delete.new('phone-credit-postpaid', Time.now, user_id, params).run!
