...
module Channel
  module NewBNI
    module Request
      #  PaymentNonBNI will process credit card bill transaction with biller non-BNI based on transaction parameter
      #  parameters:
      #  CreditCardBillTransaction: <CreditCardBillTransaction> # credit card bill transaction model
      #
      #  response from PaymentNonBNI
      #  - {} object of ResponseGeneralizer::CreditCardBill.
      class PaymentNonBNI < Base
        include ApplicationHelper

        BNI_ERROR_NUMBER_TIMEOUT = ['9126', '0008'].freeze
        TRANSACTION_URL = Channel::Config::NEW_BNI_PAYMENT_NON_BNI_URL
        ...
        def perform
          ...
          parsed_response = response.dig("soapenv:Body", "cr:paymentResponse", :response)
          result = ResponseGeneralizer::CreditCardBill.new do |res|
            res.status = SUCCESS
            res.response_code = BNI_SUCCESS
            res.partner_journal_number = parsed_response.dig(:journal)
        ...
        def validate_payment_response(response)
          ...
          return {} if response[:"soapenv:Body"] && !response.dig(:"soapenv:Body", :"soapenv:Fault")
