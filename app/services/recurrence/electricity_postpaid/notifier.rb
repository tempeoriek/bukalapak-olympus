module Recurrence
  module ElectricityPostpaid
    class Notifier < Recurrence::Postpaid::Notifier
      ...
      def template_detail
        @template_detail ||= ElectricityPostpaidRecurrenceTemplateDetail.find(@template_detail_id)
      end
      ...
      def get_balance_onsite_notif_payload(buyer_id)
        {
          ...
          body: {
            ...
            body: '2 hari lagi jadwal transaksi rutin Listrik Pascabayar. Pastikan saldo BukaDompet kamu cukup, ya.',
            image: nil,
            url: "#{ENV["BUKALAPAK_WEB_URL"]}/payment/invoices",
            tag: 'electricity-postpaid-recurrent-balance-reminder'
          }
        }
      end
      ...
      def get_balance_push_notif_payload(buyer_id)
        {
          ...
          contents: '2 hari lagi jadwal transaksi rutin Listrik Pascabayar. Pastikan saldo BukaDompet kamu cukup, ya.',
          tag: 'electricity-postpaid-recurrent-balance-reminder',
          url: "#{ENV["BUKALAPAK_WEB_URL"]}/payment/invoices"
        }
      end
      ...
      def get_buyer_id
        ::ElectricityPostpaidRecurrenceTemplateDetail.find_by(id: @template_detail_id).buyer_id
      end
    end
  end
end
